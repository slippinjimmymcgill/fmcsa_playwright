...
and download the inspection Excel export via response interception.
"""
...
async def download_sms_inspection_excel(dot_number: str, headless: bool = True) -> str:
    ...
    async with async_playwright() as p:
        ...
        print(f"[SMS] Setting up response interception...")
        ...
        async def handle_response(response):
            ...
            if (
                ...
                print(f"[SMS] Intercepted: {content_type} | {content_disp}")
                download_response = response

        page.on("response", handle_response)

        # Dispatch click (bypasses visibility)
        print(f"[SMS] Clicking Download button...")
        await btn.dispatch_event("click")

        # Wait up to 30s for intercepted response
        for _ in range(60):
        ...
        if download_response is not None:
            ...
            with open(file_path, "wb") as f:
                f.write(body)
            await browser.close()
            print(f"[SMS] Saved via interception: {file_path} ({len(body)} bytes)")
            return file_path

        # Fallback: expect_download
        print(f"[SMS] Interception failed, trying expect_download fallback...")
        try:
